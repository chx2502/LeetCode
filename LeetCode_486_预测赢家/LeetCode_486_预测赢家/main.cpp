...
//    }
    ...
    bool PredictTheWinner(vector<int>& nums) {
        ...
        const unsigned long length = nums.size();
        if (length == 1) return true;
        vector<vector<status*>> status_matrix(length);
        // 初始化状态矩阵
        for (int i = 0; i < length; i++) {
            for (int j = 0; j < length; j++) {
                if (i == j) status_matrix[i].push_back(new status(nums[i], 0));
        ...
        for (int k = 1; k < length; k++) {
            ...
            for (int start = 0; start < length-k; start++) {
                int end = start + k;
                int left = nums[start] + status_matrix[start+1][end]->second;
                int right = nums[end] + status_matrix[start][end-1]->second;
